...
export default function Village() {
    ...
    const [ajoutVill, setajoutVill] = useState(initialState);

    const handleInputChange = event => {
        const { name, value } = event.target;
        setajoutVill({ ...ajoutVill, [name]: value });
    };
    ...
    const saveVillage = () => {
        ...
        useEffect(() => {
            async function postData() {
                ...
                const postVillage = await axios.post('http://localhost:8080/Village', ajoutVill)
                    .then(response => setVillage({
                        id: response.data.id,
            ...
            <div>
                ...
                <form>
                    <div className="mb-3">
                        ...
                        <input type="text" className="form-control" id="name" aria-describedby="emailHelp" onChange={handleInputChange} name="name" value={ajoutVill.name} />
                    ...
                    <button type="submit" className="btn btn-primary" onClick={saveVillage}>Submit</button>
                </form>
            </div >
        )
    }
